...
def save_to_supabase(
    ...
    try:
        ...
        # Upsert post
        supabase.table('seo.posts').upsert(
            post_data,
            on_conflict='slug'
        ).execute()

        # Delete old links for this post
        supabase.table('seo.internal_links').delete().eq(
            'source_slug', analysis.slug
        ).execute()

        # Insert new links
        for link in analysis.internal_link_targets:
            supabase.table('seo.internal_links').insert({
                'source_slug': analysis.slug,
        ...
        supabase.table('seo.issues').delete().eq(
            'post_slug', analysis.slug
        ).execute()

        # Insert new issues
        for issue in analysis.issues:
            supabase.table('seo.issues').insert({
                'post_slug': analysis.slug,
